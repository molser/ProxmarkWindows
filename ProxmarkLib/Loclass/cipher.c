...
void MAC1(uint8_t* k, BitstreamIn input, BitstreamOut out)
{
	uint8_t zeroes_32[] = { 0,0,0,0 };
	BitstreamIn input_32_zeroes = { zeroes_32,sizeof(zeroes_32) * 8,0 };
	State initState = suc(k, init(k), &input);
...
void doMAC_N(const uint8_t *address_data_p, uint8_t address_data_size, uint8_t *div_key_p, uint8_t mac[4])
{
